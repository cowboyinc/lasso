...
- **Lasso identifies itself to Cattle Guard.** Every request carries `cowboy-client: lasso` plus version and platform tokens, which the run viewer and request traces record. Turns ask for the `agentic` model role so Cattle Guard resolves a concrete model and harness route instead of the unrouted default box.
- **Cattle Guard is now the native harness transport.** Canyon projects infer the matching Cattle Guard and Dashboard origins from their RPC URL. Plain-text prompts authenticate directly with an hourly wallet proof, negotiate protocol 1.4, stream/replay durable events until a terminal run status, and service the harness's canonical `local_list`, `local_read`, and `local_write` callbacks through the existing sandbox and approval policy. Dashboard remains the product/history layer: Lasso creates wallet-proven conversations and registers native Cattle Guard runs there, but no longer sends the harness turn through Dashboard. Unknown RPC environments fail closed instead of silently crossing into a hosted backend.
- **Protected transaction signing.** Signature requests are fetched from Cattle Guard's protected request endpoint and checked against their public event. Lasso then requires approval and calls Cowboy CLI 0.0.34+'s `transaction sign-hash --expect-tx-hex`, which decodes the exact unsigned transaction, recomputes its signing hash, and confirms the active wallet occupies a signer slot before returning `r/s/v`. Startup warns when an older CLI cannot provide this verification.
...
### Added
- **Google wallet pairing design.** `docs/GOOGLE_WALLET_PAIRING.md` defines the browser-mediated, end-to-end encrypted handoff needed for a Google-backed Dashboard wallet. Dashboard currently never possesses the Google Drive token or decrypted key, so the proposed `/login` flow uses an ephemeral Lasso ECDH key and explicit browser reauthentication instead of introducing server custody. The command and pairing endpoints remain follow-up implementation.
- **Local secret store + `/secrets` (COW-2469).** Secrets now live on your machine: values go to the macOS keychain (or a permission-locked `.cowboy/secrets.json` elsewhere), set via `/secrets set <NAME>` with a masked prompt — the value never appears on the command line, in the console log, or on the network. `/secrets list` shows names only; `/secrets delete <NAME>` removes one. The agent can ask for a secret to exist (`local_set_secret` opens the same masked prompt; cancelling refuses) and check that it exists (`local_get_secret`), but values never cross to the backend by design.
- **`/sync push` and `/sync pull` (COW-2467).** Keep the local project and the wallet's CBFS volume in both places. `push` uploads project files (skipping unchanged ones via `.cowboy/sync.json`, and never `.env*`, `.cowboy/`, `.git`, `node_modules`, or symlinks) to the volume named in the config — created on first push if missing (needs an active CBFS delegation). `pull` lists the volume, shows the full write plan (new vs overwritten files) for approval, then writes atomically through the project sandbox; remote paths that escape the project or touch protected files are refused outright, and oversized/truncated remote objects are skipped, never partially written. Auth uses an hourly wallet-signature token signed via the `cowboy` CLI, so the key never enters lasso. Conflict detection and `/sync status` land separately (COW-2468).
