...
import { acceptSuggestion, createEditorState, matchesSuggestionContext, moveSuggestionSelection, openSuggestions, shouldExitOnInterrupt } from "./editor-state.js";
    ...
    async (command: string, args: string[]) => {
      ...
      if (command === "watchtower-new-feed") {
        ...
        try {
          const cowboyArgs = commandToCowboyArgs(command, args);
          const result = await executeCowboy(cowboyArgs, session.validatorUrl);
          const feed = extractFeedEntry(result, args);
          if (feed) {
            setSession((prev) => {
